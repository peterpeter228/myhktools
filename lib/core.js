...
2、自动运行可用插件
*/
runChecks:function(url,szTags,fnCbk)
{
...
{
	console.log((fn||fnHelp).toString().split(/\n/).slice(2, -2).join('\n'));
},
	...
	if(s && url)
	{
		...
		while(a = re.exec(s))
		{
			...
			if(!oT[a[1]])
			{
				oT[a[1]] = 1;
				// console.log(url + "  " + a[1]);
				runChecks(url,"struts2,parms",{name:a[1]});
			}
		}
	}
},// 获取Ta3异常消息
...
fnDoBody:function(body,t,url,rep,fnCbk)
{
	...
	if( -1 < String(body||"").indexOf(".(#ros.flush()") ||
		-1 < String(body||"").indexOf("org.apache.commons.io.IOUtils"))return fnCbk(oRst);
	...
	if(rg1.test(body) || -1 < body.indexOf("pwd%3a") || -1 < body.indexOf("echo+whoami"))return fnCbk(oRst);
